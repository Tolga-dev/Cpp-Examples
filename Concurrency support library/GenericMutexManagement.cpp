...
void tryLock()
{
    ...
    auto inc = [&](int &c, std::mutex &m, const char *desc)
    {
        for(int i = 0; i < 10 ; i++)
        {
            // std::unique_lock<std::mutex> lock(m);
            if(std::try_lock(f_c_m,b_c_m))
            {
                c++;
    ...
    std::thread inc_f(inc, std::ref(f_c), std::ref(f_c_m), " f ");
    std::thread inc_b(inc, std::ref(b_c), std::ref(b_c_m), " b ");
